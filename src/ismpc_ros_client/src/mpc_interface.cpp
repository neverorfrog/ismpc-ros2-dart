...
void MpcInterface::step() {
    // TODO : POSTURE REGULATION TEST

    if (frame_info_.k < 1) {
        initial_state_ = state_;
        // plan_.footsteps[0].start_pose = state_.right_foot.pose.getPose2();
    }

    state_.desired_left_foot.pose = initial_state_.left_foot.pose;
    state_.desired_right_foot.pose = initial_state_.right_foot.pose;
    state_.desired_torso.pose = initial_state_.torso.pose;
    state_.desired_lip.com_pos = initial_state_.lip.com_pos;
    state_.left_foot_contact = true;
    state_.right_foot_contact = true;
    ...
    // // Torso Task
    // const auto lf_rotvec = Math::matrixToRotvec(state_.left_foot.pose.rotation.matrix());
    // const auto rf_rotvec = Math::matrixToRotvec(state_.right_foot.pose.rotation.matrix());
    ...
    // const auto lf_rotvec_dot = state_.left_foot.ang_vel;
    // const auto rf_rotvec_dot = state_.right_foot.ang_vel;
    // const auto torso_rotvec_dot = (lf_rotvec_dot + rf_rotvec_dot) / 2.0;
    // state_.desired_torso.ang_vel = torso_rotvec_dot;

    // const auto lf_rotvec_ddot = state_.left_foot.ang_acc;
    // const auto rf_rotvec_ddot = state_.right_foot.ang_acc;
