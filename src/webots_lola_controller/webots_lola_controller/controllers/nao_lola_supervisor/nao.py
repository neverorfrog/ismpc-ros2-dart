...
from controller import AnsiCodes, Supervisor

import os
import argparse
from webots_lola_controller.controllers.nao_lola_supervisor.imageserver import ImageServer, CamIndex
...
class Nao (Supervisor):
    ...
    def __init__(self, logger):
        ...
        self.logger = logger
        self.tick = 0
        self.tf = self.getFromDef("Blue5").getField("translation")
        ...
        if self.args.camera:
            self.logger.info("Starting image servers")
    ...
    def updateSensors(self):
        ...
        if self.key == ord('C'):
            if self.sensors["Touch"][0] < 1:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Chest button pressed." + AnsiCodes.RESET)
            self.sensors["Touch"][0] = 1.0
        elif self.key == ord('U'):
            if self.sensors["Touch"][2] < 1:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Head touched." + AnsiCodes.RESET)
            for i in range(1,4):
                self.sensors["Touch"][i] = 1.0
        elif self.key == ord('A'):
            if self.sensors["Touch"][1] < 1:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Front head & chest button pressed." + AnsiCodes.RESET)
            self.sensors["Touch"][0] = 1.0
            self.sensors["Touch"][1] = 1.0
        else:
            if self.sensors["Touch"][0] > 0:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Chest button released." + AnsiCodes.RESET)
            elif self.sensors["Touch"][2] > 0:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Head released." + AnsiCodes.RESET)
            elif self.sensors["Touch"][1] > 0:
                self.logger.info(AnsiCodes.CYAN_FOREGROUND + "Front head & chest button released." + AnsiCodes.RESET)
            for i in range(0,4):
    ...
    def print_status(self):
        self.logger.info(AnsiCodes.YELLOW_FOREGROUND)
        self.logger.info("Webots LoLa controller listening on:")
        self.logger.info(f"    LoLa:       {self.format_addr(self.args.lola_addr)} ")
        if self.args.camera:
            self.logger.info(f"    Top Camera:   {self.format_addr(self.args.tcam_addr)}")
            self.logger.info(f"    Bottom Camera: {self.format_addr(self.args.bcam_addr)}")
        self.logger.info("Options:  Camera={}  Framerate={}  SendTicks={}".format(self.args.camera, self.args.fps, self.args.send_ticks))
        self.logger.info(" ")
        self.logger.info("Button interface (3D view must have focus):")
        self.logger.info("     Press 'C' to simulate chest button")
        self.logger.info("     Press 'U' to simulate unstiff gesture (all head sensors touched)")
        self.logger.info("     Press 'A' to simulate calibration gesture (front head sensor touched & chest button pressed)")
        self.logger.info(AnsiCodes.RESET + " ")